function solitons(;
  ...
  for gamma_param in gamma_param_list
    ...
    offset *= 0.0
    @. analytical_gs =
      sqrt(gamma_param / 2) * 2 /
      (exp(gamma_param * (x - offset)) + exp(-(x - offset) * gamma_param))
    ...
    time_requirement[3] = @elapsed begin
      # estimate width
      initial_sigma_improved = sqrt(
        sum(abs2.(sim_gpe_1d.X[1]) .* abs2.(gpe_1d) * sim_gpe_1d.dV) |> real,
      )
      if take_advantage
    ...
    saveplots ? savefig(p, "media/" * string(gamma_param) * "_ground_states.pdf") :
    nothing
    ...
    saveplots ? savefig(p, "media/" * string(gamma_param) * "_ground_states_zoom.pdf") :
    nothing
  end
  human_readable_gs(gs_dict)
  return gs_dict
end
