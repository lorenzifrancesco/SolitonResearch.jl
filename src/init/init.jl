...
function load_parameters_alt(
    ...
    L = (Lx,)
    N = (N_axial_1D,)
    sim = Sim{length(L), Array{Complex{Float64}}}(
      L=L, 
      N=N, 
      )
    sim.iswitch = iswitch_all
    sim.equation = GPE_1D
    sim.manual = true
    ...
    sim.dt = dt_all
    sim.time_steps = Int(floor((sim.tf-sim.ti)/sim.dt))
    # specs for GS sim
    sim.maxiters = maxiters_1d

    # SPR condensate bright soliton t in units of omega_perp^-1
    analytical_gs = zeros(N)
    @. analytical_gs = sqrt(gamma_param/2) * 2/(exp(gamma_param*x) + exp(-x*gamma_param)) 
    @. sim.psi_0 = exp(-(x-x0)^2/initial_width) * exp(-im*(x-x0)*vv)
    sim.psi_0 = sim.psi_0 / sqrt(ns(sim.psi_0, sim))
    kspace!(sim.psi_0, sim)
  
    if "G1" in eqs
        push!(sim_dictionary, "G1" => deepcopy(sim))
    end
    ...
    if "N" in eqs
        push!(sim_dictionary, "N" => deepcopy(sim))
    end
    # =========================================================
    ## NPSE_plus
    sim.equation = NPSE_plus
    if gamma_param > 2/3
        @warn "we should expect NPSE collapse"
    end
    sim.sigma2 = init_sigma2(sim.g)
    if "Np" in eqs
        push!(sim_dictionary, "Np" => deepcopy(sim))
    end
    # # =========================================================
    # ## 3D-GPE 
    # L = (Lx,Lt,Lt)
...
function load_parameters_alt_old(
    ; vv::Float64 = 0.0,
    bb::Float64 = 0.0, 
    gamma_param::Float64=0.65,
    Nsaves::Int64=200,
